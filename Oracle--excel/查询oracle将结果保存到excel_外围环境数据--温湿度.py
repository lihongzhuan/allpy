...
IHTODB.TBL_IHTODB A
WHERE
A.DATE_TIME BETWEEN to_date('2020-11-3 00:00:00','YYYY-MM-DD HH24:MI:SS') AND to_date(sysdate+1) AND
...
#     print ("d%",row) 
    for col,field in enumerate(data):
        sheet.write(row,col,field,forma)
        #print(type(row))
        #print（row）
        #print（col）
        #print（field）
...
# sheet.set_column(1,None,forma)
sheet2.set_column("A:Z", 40)    #设置列宽度
sheet2.set_row(0, 30)    #设置行高度
# format = book.add_format({'color':'red'})    #获取单元格属性
#{'bold': True, 'font_size': 14, 'align': 'center','valign': 'vcenter','border':1, 'color':'red', 'bg_color':'blue'}
dir(format)    #可以显示属性的种类
# format.set_bold("A:A")    #设置粗体
