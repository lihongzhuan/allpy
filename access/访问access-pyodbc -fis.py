...
##cnxn = pyodbc.connect(r'DRIVER={Microsoft Access Driver (*.mdb, *.accdb)};DBQ=G:\\!2020数据分析模板\\现场FIS+MEA数据.accdb')
# cursor()使用该连接创建（并返回）一个游标或类游标的对象
##crsr = cnxn.cursor()



db_file = r'G:\98数据库\19领班报表\2020\数据库5.mdb'
db_file_location = r'G:\!2020数据分析模板\现场FIS+MEA数据.accdb'
# 这里用的是Python3.5的语法，如果是低版本Python的话需要改成普通方式
cnxn = pyodbc.connect(rf'Driver={{Microsoft Access Driver (*.mdb, *.accdb)}};DBQ={db_file};')
crsr = cnxn.cursor()
...
print('`````````````` 数据表名称列表``````````````')
for table_info in crsr.tables(tableType='Query'):
...
# 打印查询的结果
for row in crsr.execute("SELECT * from AAA_FISEXCLUSIONNO"):
    print(row)
